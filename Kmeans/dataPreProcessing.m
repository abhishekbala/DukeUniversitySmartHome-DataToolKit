...
powerData(:,1) = sum(shData(:,2:3),2) - sum(shData(:,4:5),2); % phase1+phase2-Solar
powerData(:,2) = shData(:,6) + shData(:,8); % AHU_1+CU_1
powerData(:,3) = shData(:,7) + shData(:,9); % AHU_2+CU_2
powerData(:,4) = shData(:,12); % H10P
powerData(:,5) = shData(:,13); % Hot box
powerData(:,6) = shData(:,13); % Refrigerator
...
for num = 1:59
    ...
    for i = 1:n
        ...
        missPowerData(i,:) = completePowerData(missIndex(i),:);
    end
...
minMeanPower=zeros(ceil(length(completePowerData)/60)-1,6);
