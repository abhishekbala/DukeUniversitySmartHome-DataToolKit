...
% % % % % % % %     csvwrite('DisaggregatedPower.csv', myOutput);
    ...
    if(ApplianceVector(2) == 1)
        [myOutput(2), pointers(1)] = TimeSeriesCreate2(ApplianceVector, deltaPower, functionPointers(1));
    elseif(ApplianceVector(2) == 2)
        [myOutput(3), pointers(2)] = TimeSeriesCreate2(ApplianceVector, deltaPower, functionPointers(2));
    elseif(ApplianceVector(2) == 3)
        [myOutput(4), pointers(3)] = TimeSeriesCreate2(ApplianceVector, deltaPower, functionPointers(3));
    elseif(ApplianceVector(2) == 4)
        [myOutput(5), pointers(4)] = TimeSeriesCreate2(ApplianceVector, deltaPower, functionPointers(4));
    end
...
elseif(pointerStatus == 1 && onOffStatus == 0) % PREVIOUS STATUS OF APPLIANCE IS ON AND CURRENT STATUS SAYS APPLIANCE STAYS IN ORIGINAL STATE
    ApplianceTimePoint = avgOnPower; % Keep the appliance power output to ON.
    pointer = 1;
