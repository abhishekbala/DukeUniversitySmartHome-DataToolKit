function trainingOnAggregate(submeteredDataSet, aggregateDataSet, applianceLabel, decisionIDs, parameters)

    dataLength = length(submeteredDataSet);
    ...
    f = parameters(6);
    g = parameters(7);
    [on, off, events] = GLR_EventDetection(submeteredDataSet, a, b, c, d, e, f, g);
    ...
    for i = 1:dataLength
        if on(i) == 1
            eventWindow = aggregateDataSet(i-trainingWindow:i+trainingWindow)';
        ...
        if off(i) == 1
            eventWindow = submeteredDataSet(i-trainingWindow:i+trainingWindow)';
            eventSlope = polyfit(1:21,eventWindow,1);
            eventDelta = max(eventWindow) - min(eventWindow);
